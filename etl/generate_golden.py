...
from percentiles import POSITION_CONFIGS, _aggregate_season
...
PLAYERS = [
    ...
    {
        # Screen-only tight end: net NEGATIVE receiving air yards. Python
        # gates RACR on air yards > 0, so the season RACR is null (excluded
        # from ranking) rather than a negative conversion ratio.
        "playerId": "te-negative-air-yards",
        "position": "TE",
        "weeks": [
            {"week": 1, "targets": 5, "receptions": 5, "receivingYards": 40,
             "receivingYac": 45, "receivingAirYards": -5, "targetShare": 0.12,
             "airYardsShare": -0.02, "wopr": 0.17, "receivingEpa": 1.4},
            {"week": 2, "targets": 4, "receptions": 3, "receivingYards": 20,
             "receivingYac": 25, "receivingAirYards": -8, "targetShare": 0.09,
             "airYardsShare": -0.03, "wopr": 0.11, "receivingEpa": -0.6},
        ],
    },
    {
        # Same shape on the passing side: negative total passing air yards
        # nulls PACR while aDOT stays a real (negative) number.
        "playerId": "qb-negative-air-yards",
        "position": "QB",
        "weeks": [
            {"week": 1, "attempts": 10, "completions": 8, "passingYards": 50,
             "passingAirYards": -12, "sacksSuffered": 0, "sackYardsLost": 0,
             "passingCpoe": 4.0, "passingEpa": 1.2},
        ],
    },
    {
        # One-week tiny sample: every ratio must still resolve.
        "playerId": "wr-single-week",
        "position": "WR",
        "weeks": [
            {"week": 7, "targets": 1, "receptions": 1, "receivingYards": 9,
             "receivingYac": 4, "receivingAirYards": 6, "targetShare": 0.05,
             "airYardsShare": 0.04, "wopr": 0.10, "receivingEpa": 0.7},
        ],
    },
    {
        # Played but every stat null (e.g. injured on the first snap): all
        # rates are null, all counts are 0, and nothing is NaN.
        "playerId": "rb-all-null",
        "position": "RB",
        "weeks": [
            {"week": 1},
            {"week": 2},
...
COMPARED_METRICS["TE"] = COMPARED_METRICS["WR"]
...
def build_golden() -> dict:
    """Assemble the golden document. Pure: no file writes, no network."""
    df = build_dataframe()
    ...
    return {
        ...
        # The ranking configuration is exported too, so the frontend can
        # assert its per-position row order and the qualifier thresholds it
        # prints in the Advanced footnote match what the ETL actually ranks.
        "positionConfigs": {
            position: {
                "metrics": list(config["metrics"]),
                "qualifierMin": config["qualifier_min"],
...
def main() -> None:
    ...
    print(f"Players: {len(PLAYERS)}, expected maps: {len(golden['expected'])}")
