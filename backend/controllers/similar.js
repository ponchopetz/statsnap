...
// GET /players/:playerId/similar?season=YYYY&limit=N
// Nearest percentile profiles in the same position cohort and season.
...
const getSimilarPlayers = async (req, res, next) => {
  ...
  try {
    ...
    if (!target.advanced) {
      return res.status(200).json({ playerId, season, position: target.position, qualified: false, similar: [] });
    }

    const cohort = await PlayerStats.find(
      { season, position: target.position, advanced: { $exists: true }, playerId: { $ne: playerId } },
      PROJECTION,
    ).lean();
    ...
    res.status(200).json({ playerId, season, position: target.position, qualified: true, similar });
  } catch (err) {
    next(err);
  }
};
